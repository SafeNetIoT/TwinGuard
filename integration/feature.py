...
# === Config ===
INPUT_FOLDER = "../data/integration/daily"
OUTPUT_FOLDER = "../data/integration/encoded"
...
# === Sensitive Words ===
with open("../data/base/sensitive_words_with_counts.txt", "r") as f:
...
def process_file(input_path, output_path):
    ...
    HTTP_METHODS = ["GET", "POST", "OPTIONS", "HEAD", "PRI", "PROPFIND", "KGSY", "CONNECT", "LFIM", "HBLB", "XPDU", "PUT", "DELETE", "SSTP_DUPLEX_POST", "PDWB", "RXRM", "VFTF", "CENG", "UKUP", "IXNJ", "JIPW", "OMAC", "EOFQ", "KJLE", "TWUT", "AMJL", "NZWJ", "ZVGU", "ZGKC", "UFSR", "QOQJ", "UQSI", "ARYC", "WLGK", "ZSDLKFJ2", "OEHR", "NMRE", "MGLI", "KMKQ", "RCIJ", "OLNL", "QPOH", "BIPU", "CYFD", "PATCH", "ELEJ", "YPYJ", "KVFA", "REHE", "TRACE", "OTHER"]
    ...
    # Save Combined Label Mapping
    label_mapping_path = os.path.join(OUTPUT_FOLDER, "label_mapping.json")
    method_labels = {str(k): v for k, v in zip(le_method.transform(le_method.classes_), le_method.classes_)}
    category_labels = {str(k): v for k, v in zip(le_cat.transform(le_cat.classes_), le_cat.classes_)}
    combined_mapping = {"http_method": method_labels, "category": category_labels}

    with open(label_mapping_path, "w") as f:
        json.dump(combined_mapping, f, indent=2)
    print("Saved combined label mapping to label_mapping.json")
    ...
    drop_cols = [
        "protocol", "http_accept", "http_accept_encoding", "http_user_agent", "http_content_type",
        ...
        "host_asn", "host_as_org", "host_country", "user_agent_categories", "uri_tokens", "body_tokens"
    ]
...
# === Loop over date range for X-POT data ===
start_dt = datetime.strptime(START_DATE, "%Y-%m-%d")
end_dt = datetime.strptime(END_DATE, "%Y-%m-%d")

while start_dt <= end_dt:
    day_str = start_dt.strftime("%Y-%m-%d")
    in_path = os.path.join(INPUT_FOLDER, f"test_day_{day_str}.csv")
    out_path = os.path.join(OUTPUT_FOLDER, f"test_day_{day_str}_encoded.csv")

    if os.path.exists(in_path):
        process_file(in_path, out_path)
    else:
        print(f"⚠️ File missing: {in_path}")
