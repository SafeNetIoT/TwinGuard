...
TRIE_PATH = "../data/trie/aggregated_probabilistic_attack_tree.json"
SENSITIVE_PATH = "../data/trie/sensitive_words_with_counts.txt"
...
LABEL_COL = "category"
MODEL_SAVE_FOLDER = "adaptive_model"
start_date = "2025-03-15"
end_date = "2025-04-09"
RESULTS_FOLDER = f"../data/dt/digital_twin_log_win{WINDOW_SIZE}.csv"
...
def load_day_data(day_str):
    path = os.path.join(ENCODED_FOLDER, f"http_day_{day_str}_encoded.csv")
    if os.path.exists(path):
        with warnings.catch_warnings():
            warnings.simplefilter(action='ignore', category=pd.errors.DtypeWarning)
            df = pd.read_csv(path)
        return df[df[LABEL_COL].isin(CATEGORY_ID_TO_NAME.keys())]
    return None
...
def build_probabilistic_trie(df, sensitive_words):
    ...
    df["attack_sequence"] = list(zip(df["http_method"], df["http_status"], df["uri_keywords"]))
...
def build_sequence(row, sensitive_words):
    method_id = row["http_method"]
    method_str = METHOD_ID_TO_NAME.get(int(method_id), str(method_id))  # Map method ID back to string
    status_raw = row["http_status"]
    status = str(int(status_raw)) if pd.notna(status_raw) and str(status_raw).replace('.', '', 1).isdigit() else "Unknown"
...
def run_adaptive_loop(start_date, end_date):
    ...
    window_df = [pd.read_csv(os.path.join(DAILY_RAW_FOLDER, f"http_day_{day}.csv")) for day in train_days]
