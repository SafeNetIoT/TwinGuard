...
# --- Load Data ---
stable = pd.read_csv("../data/vir/stable_model_metrics_rf_w8.csv")
static = pd.read_csv("../data/vir/static_model_metrics_rf_w8.csv")
...
# --- Plot with updated size and spacing ---
...
# X-ticks: show every n days, month-day only, no rotation
every_n = 14
xticks = stable_daily["date"].iloc[::every_n]
ax1.set_xticks(xticks)
ax1.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d'))
plt.setp(ax1.get_xticklabels(), fontsize=24, rotation=0, ha='center')

# Drift on secondary Y-axis
ax2 = ax1.twinx()
...
ax3.tick_params(axis='y', labelcolor="#f3a361", labelsize=20)
ax3.set_ylim(0, 1)

# === Legend (on top, one line, no border) ===
lns = ln1 + ln2 + ln3 + ln4
...
plt.savefig("../data/vir/model_overall.pdf", bbox_inches='tight')  # Save as .pdf
plt.show()
...
pd.DataFrame(overall).T.to_csv("../data/vir/highlight_overall_static_vs_stable.csv")
pd.DataFrame(retrain_only).T.to_csv("../data/vir/highlight_retrain_static_vs_stable.csv")
with open("../data/vir/highlight_rescue_rate.txt", "w") as f:
    f.write(rescue_str)
